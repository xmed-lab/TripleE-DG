...
class MoCo(nn.Module):
    ...
    def __init__(self, base_encoder, dim=128, K=65536, m=0.999, T=0.07, mlp=False, args=None):
        ...
        self.fc = self.encoder_q.fc
        self.fc2 = self.encoder_q.fc2
    ...
    def forward(self, im_q, im_k, domain_idx, trainflag):
        ...
        probabilities, q, _ = self.encoder_q(im_q)  # queries: NxC
        ...
        if trainflag:
            ...
            b = torch.reshape(a, (
            int(self.K / 16), 4, 4))  # how many block pushes, each block repeat times, four sampled ID
