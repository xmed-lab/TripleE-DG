...
from common_lib import get_train_standard_transformers, \
...
def main_worker(args):
    ...
    dataset = get_dataset(dataset="camelyon17", root_dir="/home/eexmli/193dataset/WILDS/", download=False)

    # Get the training set
    from torchvision import transforms
    datasets_train = dataset.get_subset(
        "train",
        transform=get_train_ESAugSole_transformers(),
    )
    train_loader = get_train_loader("standard", datasets_train, batch_size=16)
    ...
    val_loader = get_eval_loader("standard", val_data, batch_size=16, drop_last=False)
    ...
    test_loader = get_eval_loader("standard", test_data, batch_size=16, drop_last=False)

    print("Dataset size: train %d" % (len(train_loader.dataset)))
    print("Dataset size: val %d" % (len(val_loader.dataset)))
...
def train(train_loader, model, criterion, ce_criterion, optimizer, args):
    ...
    for data, class_l, _ in train_loader:
